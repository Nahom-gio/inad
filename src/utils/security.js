  ...
  switch (type) {
    ...
    case 'name':
      ...
      if (!nameRegex.test(sanitized)) {
        return { isValid: false, value: '', error: 'Name contains invalid characters' };
      }
      if (sanitized.length < 2) {
        return { isValid: false, value: '', error: 'Name must be at least 2 characters' };
      }
      if (sanitized.length > 50) {
        return { isValid: false, value: '', error: 'Name must be less than 50 characters' };
      }
      break;
      
    case 'company':
      if (sanitized.length > 100) {
        return { isValid: false, value: '', error: 'Company name must be less than 100 characters' };
      }
      const companyRegex = /^[a-zA-Z0-9\s&.,'-]+$/;
      if (!companyRegex.test(sanitized)) {
        return { isValid: false, value: '', error: 'Company name contains invalid characters' };
      }
      break;
      
    case 'message':
      if (sanitized.length < 10) {
        return { isValid: false, value: '', error: 'Message must be at least 10 characters' };
      }
      if (sanitized.length > 1000) {
        return { isValid: false, value: '', error: 'Message must be less than 1000 characters' };
      }
      break;
  }
